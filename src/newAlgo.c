...
// Calculates a lot of boards states
void initBoardsArr(Board* baseBoard) {

	boardsArr = (Board**) malloc(MAX_BOARDS * sizeof(Board*));
	checkNullPointer((void*) boardsArr);

	boardsArr[0] = baseBoard;
	
	for(int i = 1; i < MAX_BOARDS; i++) {
		boardsArr[i] = copyBoard(boardsArr[i - 1]);
		updateBoard(boardsArr[i], 0, 0);
	}
}

// Free all boards calculated before
void freeBoardsArr() {
	for(int i = 0; i < MAX_BOARDS; i++)
		freeBoard(boardsArr[i]);
}
...
void autoPlay(Board* currentBoard, int depth) {
	...
	initBoardsArr(currentBoard);

	for(int i = 0; i < MAX_BOARDS; i++)
		printBoard(boardsArr[i]);
